...
aucs = c()
pdf(file.path(data_dir,'plot.rocs.pdf'), width=20, height=15)  # ROC curves in Figure 1 of manuscript
par(mfrow=c(4, 5))
...
# Figure 1 - ROC curves and AUC differences between reg-eQTL and s-eQTL methods across various MAF categories and effect size combinations 
nms <- c("-0.6:0.1:-0.2", "0.6:0.1:0.3","0.6:-0.1:0.3","0.6:0.1:0.3",
					"-0.5:0.1:0.1", "-0.3:0.1:0", "-0.6:-0.1:0.2", "-0.1:-0.1:-0.3")
mafs <- c("A","B","C","D",
	         "A","B","C","D")
roc_plots <- list()
for(n in 1:length(nms)){
	...
	df.sim = list.by.maf.sim[[m]]
	# df.sim$tf.snp.eff = paste(df.sim$TF_EFF, df.sim$TF_EFF_ADD, df.sim$SNP_EFF, sep=':')
	df.sim$tf.snp.eff = paste(df.sim$TF_EFF, df.sim$SNP_EFF, df.sim$TF_EFF_ADD, sep=':')
	df.sim.list = split(df.sim, df.sim$tf.snp.eff)

	df.reg = list.by.maf.reg[[m]]
	# df.reg$tf.snp.eff = paste(df.reg$TF_EFF,  df.reg$TF_EFF_ADD, df.reg$SNP_EFF,  sep=':')
	df.reg$tf.snp.eff = paste(df.reg$TF_EFF,  df.reg$SNP_EFF, df.reg$TF_EFF_ADD,  sep=':')
	df.reg.list = split(df.reg, df.reg$tf.snp.eff)
	...
	# ggroc
	gg <- ggroc(list(`reg-eQTL` = roc_curve_reg, `s-eQTL` = roc_curve),size=1.5)
	roc_plots[[n]] <- gg + 
	    geom_abline(intercept = 1, slope = 1,
	                color = "darkgrey", size=1) +
	  	labs(title = paste0("[", m, ':', nm, ']') ,  
	       x = "Specificity",
	       y = "Sensitivity") +
			scale_colour_manual(labels = c(paste0("reg-eQTL, AUC=",round(roc_curve_reg$auc,2)),paste0("s-eQTL, AUC=",round(roc_curve$auc,2))),values = c("red", "blue")) +
	  	theme_classic() +  
		  theme(
...
pdf(file.path(data_dir,'roc_plots.pdf'), width=30, height=15)
print(ggarrange(roc_plots[[1]], roc_plots[[2]], roc_plots[[3]], roc_plots[[4]],
					roc_plots[[5]], roc_plots[[6]], roc_plots[[7]], roc_plots[[8]],
					ncol = 4, nrow = 2))
dev.off()


# Figure 2 - Heatmap of the number of models outperformed by reg-eQTL and s-eQTL across different simulated effect sizes and directions
auc.sig.df <- auc.df[which(auc.df$delong.pval < 0.05), ]
auc.sig.df <- auc.sig.df[order(auc.sig.df$delong.pval), ]

auc.sig.reg <- auc.sig.df[which(auc.sig.df$color == "+"), ]
auc.sig.reg <- auc.sig.reg[order(auc.sig.reg$diff, decreasing=TRUE), ]

auc.sig.sim <- auc.sig.df[which(auc.sig.df$color == "-"), ]
auc.sig.sim <- auc.sig.sim[order(auc.sig.sim$diff), ]

# Figure 2A
reg_counts_tbl <- as.data.frame(table(auc.sig.reg$tf.eff, auc.sig.reg$snp.eff))
names(reg_counts_tbl) <- c("tf.eff", "snp.eff", "count")
reg_counts_tbl$snp.eff <- as.numeric(as.character(reg_counts_tbl$snp.eff))
fill_limits <- c(0, 27) # max(reg_counts_tbl_2$count)
reg_counts_plot <- ggplot(reg_counts_tbl, aes(x = tf.eff, y = snp.eff)) +
  geom_tile(aes(fill = count)) +  
  # geom_text(aes(label = count), color = "black", size=6) +  
  scale_fill_gradient(low = "white", high = "red", limits = fill_limits) +  
  labs(title = "", x = "TF Effect", y = "SNP Effect", fill = "Count") +  
  theme_classic() +  
	  theme(
...
# Figure 2B
reg_counts_tbl_2 <- as.data.frame(table(auc.sig.reg$tf.eff.add, auc.sig.reg$snp.eff))
names(reg_counts_tbl_2) <- c("tf.eff.add", "snp.eff", "count")
reg_counts_tbl_2$snp.eff <- as.numeric(as.character(reg_counts_tbl_2$snp.eff))
reg_counts_plot_2 <- ggplot(reg_counts_tbl_2, aes(x = tf.eff.add, y = snp.eff)) +
  geom_tile(aes(fill = count)) +  
  # geom_text(aes(label = count), color = "black", size=6) +  
  scale_fill_gradient(low = "white", high = "red", limits = fill_limits) +  
  labs(title = "", x = "TF:SNP Effect", y = "SNP Effect", fill = "Count") +  
  theme_classic() +  
	  theme(
	  plot.title = element_text(size = 20, hjust = 0.5),
	  axis.title.x = element_text(size = 20),
...
# Figure 2C
reg_counts_tbl_3 <- as.data.frame(table(auc.sig.reg$tf.eff, auc.sig.reg$tf.eff.add))
names(reg_counts_tbl_3) <- c("tf.eff", "tf.eff.add", "count")
reg_counts_plot_3 <- ggplot(reg_counts_tbl_3, aes(x = tf.eff, y = tf.eff.add)) +
  geom_tile(aes(fill = count)) +  
  # geom_text(aes(label = count), color = "black", size=6) +  
  scale_fill_gradient(low = "white", high = "red", limits = fill_limits) +  
  labs(title = "", x = "TF Effect", y = "T:SNP Effect", fill = "Count") +  
  theme_classic() +  
	  theme(
	  plot.title = element_text(size = 20, hjust = 0.5),
	  axis.title.x = element_text(size = 20),
...
# Figure 2D
sim_counts_tbl <- as.data.frame(table(auc.sig.sim$tf.eff, auc.sig.sim$snp.eff))
names(sim_counts_tbl) <- c("tf.eff", "snp.eff", "count")
sim_counts_tbl$snp.eff <- as.numeric(as.character(sim_counts_tbl$snp.eff))
sim_counts_plot <- ggplot(sim_counts_tbl, aes(x = tf.eff, y = snp.eff)) +
  geom_tile(aes(fill = count)) +  
  # geom_text(aes(label = count), color = "black", size=6) +  
  scale_fill_gradient(low = "white", high = "blue", limits = fill_limits) +  
  labs(title = "", x = "TF Effect", y = "SNP Effect", fill = "Count") +  
  theme_classic() +  
	  theme(
	  plot.title = element_text(size = 20, hjust = 0.5),
...
# Figure 2E
sim_counts_tbl_2 <- as.data.frame(table(auc.sig.sim$tf.eff.add, auc.sig.sim$snp.eff))
names(sim_counts_tbl_2) <- c("tf.eff.add", "snp.eff", "count")
sim_counts_tbl_2$snp.eff <- as.numeric(as.character(sim_counts_tbl_2$snp.eff))
sim_counts_plot_2 <- ggplot(sim_counts_tbl_2, aes(x = tf.eff.add, y = snp.eff)) +
  geom_tile(aes(fill = count)) +  
  # geom_text(aes(label = count), color = "black", size=6) +  
  scale_fill_gradient(low = "white", high = "blue", limits = fill_limits) +  
  labs(title = "", x = "TF:SNP Effect", y = "SNP Effect", fill = "Count") +  
...
# Figure 2F
sim_counts_tbl_3 <- as.data.frame(table(auc.sig.sim$tf.eff, auc.sig.sim$tf.eff.add))
names(sim_counts_tbl_3) <- c("tf.eff", "tf.eff.add", "count")
sim_counts_plot_3 <- ggplot(sim_counts_tbl_3, aes(x = tf.eff, y = tf.eff.add)) +
  geom_tile(aes(fill = count)) +  
  # geom_text(aes(label = count), color = "black", size=6) +  
  scale_fill_gradient(low = "white", high = "blue", limits = fill_limits) +  
  labs(title = "", x = "TF Effect", y = "TF:SNP Effect", fill = "Count") +  
  theme_classic() +  
	  theme(
	  plot.title = element_text(size = 20, hjust = 0.5),
	  axis.title.x = element_text(size = 20),
...
# Figure 3 - Performance of reg-eQTL and s-eQTL. at a q-value threshold of 0.05 across all simulations
# Figure 3A, 3B, 3C - Histogram showing the accuracy 
m1 <- metrics.df[which(metrics.df$q_thr == 0.05), ]
m1.reg <- m1[ ,c("maf","reg.sens","reg.spec","reg.acc")]
colnames(m1.reg) <- c("maf","Sensitivity", "Specificity", "Accuracy")
m1.sim <- m1[ ,c("maf","sim.sens","sim.spec","sim.acc")]
colnames(m1.sim) <- c("maf","Sensitivity", "Specificity", "Accuracy")

m1.both <- rbind(m1.reg,m1.sim)
m1.both.2 <- pivot_longer(m1.both,
                        cols = c(Sensitivity, Specificity, Accuracy),
                        names_to = "metrics",
                        values_to = "value")
both.hist <- ggplot(data = m1.both.2, aes(x = value)) +
  geom_histogram(aes(fill = metrics), bins = 50, binwidth=0.05, alpha = 0.5, position = "identity", fill="black") +
  facet_grid(maf ~ metrics, scales = "free_y", space = "free_y",  switch = "x") +  # Separate rows and columns
  labs(title = "",
       x = "",
       y = "Frequency",
       fill = "Metrics") +
  theme_minimal() +
    theme(strip.text = element_text(size = 18),
    	 strip.text.y = element_text(size=16, angle = 0, hjust = 0.5),
...
# Figure 3E, 3F - Analysis of the impact of varying the q-value threshold on the accuracy of reg-eQTL and s-eQTL methods for trios -0.6:0.1:-0.2 for MAF <0.05 and -0.5:0.1:0.1 for MAF < 0.05.
# reg-eQTL
eff.grid = "-0.6:-0.2:0.1" 
maf= "A"
q_thr_reg = 0.82
q_thr_sim = 0.52

# s-eQTL
eff.grid = "-0.5:0.1:0.1" 
maf= "A"
q_thr_reg = 0.78
q_thr_sim = 0.99

m1 <- metrics.df[which(metrics.df$maf == maf & metrics.df$eff.grid == eff.grid), ]
m2 <- melt(m1, id.vars = c("q_thr"), measure.vars = c("sim.acc", "reg.acc"))


ggplot(m2, aes(x = q_thr, y = value)) +  
  geom_line(aes(color = variable), size=1.5) +
  geom_vline(xintercept = q_thr_reg, linetype = "dashed", color = "red", size = 1) +  # Add vertical line
  geom_vline(xintercept = q_thr_sim, linetype = "dashed", color = "blue", size = 1) +  # Add vertical line
  labs(title = "" ,  
       x = "q_thr",
       y = "Accuracy",
       color = "",
       linetype = "eff.grid") +
  scale_color_manual(values = c("sim.acc" = "blue", "reg.acc" = "red")) +
...
ggplot(m2, aes(x = q_thr, y = value)) +  
  geom_line(aes(color = variable), size=1.5) +
  geom_vline(xintercept = q_thr, linetype = "dashed", color = "blue", size = 1) +  # Add vertical line
  labs(title = "" ,  
       x = "q_thr",
       y = "Accuracy",
       color = "",
...
# Figure 4 - Systematic investigation of how performance of reg-eQTL and s-eQTL methods varied with q-value cutoff in four scenarios
m1 <- metrics.df[which(metrics.df$maf == "A" & metrics.df$q_thr %in% c(0.01, 0.05, 0.1, 0.2, 0.5,0.9,0.99)), ]
grid.select <- data.frame(tf_eff = c(0.1,0.6,0.1,0.6,0.3), snp_eff = c(0.1,0.6,0.6,0.1,0.3))
...
for(g in 1:nrow(grid.select)) {
  tf_eff <- grid.select[g, ]$tf_eff 
  snp_eff <- grid.select[g, ]$snp_eff 
  m2 <- (m1[which(m1$snp.eff == snp_eff & m1$tf.eff == tf_eff & m1$tf.eff.add %in% c(0, 0.1,0.2,0.3)), ])
  m2$tf.eff.add <- as.character(m2$tf.eff.add)
  m3 <- melt(m2, id.vars = c("q_thr", "eff.grid","tf.eff.add"), measure.vars = c("sim.sens", "reg.sens"))
  m4 <- melt(m2, id.vars = c("q_thr", "eff.grid","tf.eff.add"), measure.vars = c("sim.spec", "reg.spec"))
  m5 <- melt(m2, id.vars = c("q_thr", "eff.grid","tf.eff.add"), measure.vars = c("sim.acc", "reg.acc"))
  sens.plot[[g]] <- ggplot(m3, aes(x = q_thr, y = value)) +  
    geom_line(aes(color = variable, linetype = tf.eff.add), size=1.5) +
    labs(title = paste0("[", tf_eff,":",snp_eff,"]") ,  
         x = "q_thr",
         y = "Sensitivity",
         color = "",
         linetype = "eff.grid") +
    scale_color_manual(values = c("sim.sens" = "blue", "reg.sens" = "red")) +
    scale_linetype_manual(values = c("0" = "solid", "0.1" = "dashed","0.2" = "dotted","0.3" = "dotdash")) + 
    theme_minimal() +
    theme(
...
for(g in 1:nrow(grid.select)) {
  ...
  m4 <- melt(m2, id.vars = c("q_thr", "eff.grid","tf.eff.add"), measure.vars = c("sim.spec", "reg.spec"))
  m5 <- melt(m2, id.vars = c("q_thr", "eff.grid","tf.eff.add"), measure.vars = c("sim.acc", "reg.acc"))
	spec.plot[[g]] <- ggplot(m4, aes(x = q_thr, y = value)) +
  geom_line(aes(color = variable, linetype = tf.eff.add), size=1.5) +
  labs(title = paste0("[",tf_eff,":",snp_eff,"]"), 
       x = "q_thr",
       y = "Specificity",
       color = "",
       linetype = "eff.grid") +
  scale_color_manual(values = c("sim.spec" = "blue", "reg.spec" = "red")) +
  scale_linetype_manual(values = c("0" = "solid", 
                                   "0.1" = "dashed",
                                   "0.2" = "dotted",
                                   "0.3" = "dotdash")) + 
  ...
  axis.title.y = element_text(size = 30,face="bold"),
  axis.text.y = element_text(size = 35,face="bold"),
  legend.text = element_text(size=10), #element_blank(), 
  legend.title = element_blank(), #element_text(size=12),
  legend.key.width = unit(2, "cm"),
  legend.box = "vertical",       
	legend.box.spacing = unit(0.05, "cm"), 
...
for(g in 1:nrow(grid.select)) {
  ...
  m4 <- melt(m2, id.vars = c("q_thr", "eff.grid","tf.eff.add"), measure.vars = c("sim.spec", "reg.spec"))
  m5 <- melt(m2, id.vars = c("q_thr", "eff.grid","tf.eff.add"), measure.vars = c("sim.acc", "reg.acc"))
	acc.plot[[g]] <- ggplot(m5, aes(x = q_thr, y = value, linetype = tf.eff.add)) +
  geom_line(aes(color = variable), size=1.5) +
  labs(title = paste0("[",tf_eff,":",snp_eff,"]"), 
       x = "q_thr",
       y = "Accuracy",
